    ...
    <ImageBackground source={require('../assets/fondoPistacho.jpg')} style={styles.background}>
      ...
      <Modal transparent visible={modalVisible} animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={[styles.modalBox, modalType === 'error' ? styles.modalError : styles.modalSuccess]}>
            <FontAwesome
              name={modalType === 'error' ? 'times-circle' : 'check-circle'}
              size={50}
              color={modalType === 'error' ? '#e74c3c' : '#2e7d32'}
              style={{ marginBottom: 10 }}
            />
            <Text style={styles.modalMessage}>{modalMessage}</Text>
            <TouchableOpacity
              style={[styles.modalButton, { backgroundColor: modalType === 'error' ? '#e74c3c' : '#2e7d32' }]}
...
const styles = StyleSheet.create({
  ...
  picker: { color: '#fff', height: 45, marginLeft: Platform.OS === 'android' ? -8 : 0,},
  saveButton: {
    backgroundColor: '#fff',
    paddingVertical: 15,
    borderRadius: 10,
  ...
  modalError: { borderLeftWidth: 5, borderLeftColor: '#e74c3c' },
  modalSuccess: { borderLeftWidth: 5, borderLeftColor: '#2e7d32' },
});
